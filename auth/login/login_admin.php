...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username_value === '' || $password === '') {
        ...
        $query = "SELECT id, username, nama_lengkap, role, password, cashier_id, contact_number, email, profile_photo FROM users WHERE username = '$safe_username' AND role = 'admin' AND is_active = 1 LIMIT 1";
        $result = $conn->query($query);

        if ($result && $result->num_rows === 1) {
            $admin = $result->fetch_assoc();
            $password_check = verifyUserPasswordCompatibility($password, $admin['password'] ?? '');
            ...
            if ($password_check['verified']) {
                if (!empty($password_check['needs_rehash'])) {
                    ...
                    if ($admin_id > 0) {
                        $conn->query("UPDATE users SET password = '$new_hash' WHERE id = $admin_id LIMIT 1");
                    }
                }

                loginCashierSession($admin);
                header('Location: /aplikasi-kasir-copy/dashboard.php');
                exit;
            }
        }

        $error_message = 'Username admin atau password tidak valid.';
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Login Administrator - Kasir Pintar</title>
...
<body class="auth-page">
    ...
    <div class="auth-card login-card">
        ...
        <div class="brand-subtitle">Login Administrator</div>
