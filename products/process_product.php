...
function ensureProductsTableHealthy($conn) {
    ...
    if (!$table_check || $table_check->num_rows === 0) {
        if (!$conn->query($create_sql)) {
            throw new RuntimeException('Unable to create products table: ' . $conn->error);
        }
        return;
    }
    ...
    if ((int) $conn->errno === 1932) {
        ...
        if (!$conn->query($create_sql)) {
            throw new RuntimeException('Unable to rebuild products table: ' . $conn->error);
        }

        return;
    }

    throw new RuntimeException('Products table probe failed: ' . $conn->error);
}
